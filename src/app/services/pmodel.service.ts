...
export class PmodelService {
  ...
  getModels(): Observable<any> {
    return this.http.get<{}>(baseUrl+'cataler/get_parts/',{})
    .pipe(map(data => {
      // console.log(data);
      return data;
  ...
  addModel(model_info): Observable<any> {
    return this.http.post<any>(baseUrl+'cataler/add_part/', JSON.stringify(model_info), httpOptions)
    .pipe(map(data => {
      return data;
  }),catchError(this.hErrorService.handleError.bind(this))
  );
  ...
  updateModel(model_info): Observable<any> {
    return this.http.post<any>(baseUrl+'cataler/update_part/', JSON.stringify(model_info), httpOptions)
    .pipe(map(data => {
      return data;
  }),catchError(this.hErrorService.handleError.bind(this))
  );
